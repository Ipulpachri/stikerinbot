    ...
    if (m.isGroup) {
        if (m.mentionedJid.includes(this.user.jid)) {
            await this.send2Button(m.chat,
                isBanned ? 'SF BOTZ tidak aktif' : banned ? 'kamu dibanned' : 'SF BOTZ aktif',
                '© Fachri',
...
`.trim(), '© Fachri', 'Pemilik Bot', ',owner', m)
    }
    ...
    if (set.autoupdatestatus) {
        if (new Date() * 1 - set.status > 1000) {
            ...
            await this.setStatus(`Aktif selama ${uptime} | Mode: ${set.self ? 'Private' : set.group ? 'Hanya Grup' : 'Publik'} | SF BOTZ by Fachri`).catch(_ => _)
